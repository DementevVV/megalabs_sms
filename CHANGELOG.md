...
- Настраиваемые таймауты `open_timeout` и `read_timeout`
- Поддержка логгера `logger`
- Валидация параметров `from`, `to`, `message` и таймаутов

### Изменено

- Улучшено логирование ошибок HTTP и исключений
- Удалены неиспользуемые зависимости из gemspec
- Удалён отсутствующий `.yardopts` из списка файлов
- `send_sms` теперь поддерживает именованные аргументы (с обратной совместимостью)
